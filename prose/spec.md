...
- `add-note` (`action`): insert or replace a `@note`. A block note is addressed by the block's path (`src/store.ts#addTodo`, or `src/store.ts#file` for the file prose) and its content hash; a line note by the file and line (`src/store.ts:42`) and a hash of that line's text. A line note lands above the construct that contains the line (§6.2), so the server answers with where; a spot directly under a block's prose is that block's own, and the write goes to its block note.
- `note-target` (`query`): where a line note picked at `src/store.ts:42` would land, given the line's hash: the line it goes above, that line's text, and whether it is a block's own spot. Writes nothing; the view shows it before the note is saved.
- `resolve-note` (`action`): remove a `@note` entirely, addressed by the note itself: its file, its line, and a hash of its text. A block note is resolved by its block's path and hash, which covers the note's text.
