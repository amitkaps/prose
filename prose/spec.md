...
> **Alternative: mounting inside `@vitejs/devtools`.** An earlier pass wired `prose()` through `@vitejs/devtools-kit`'s `createPluginFromDevframe`, which mounts a devframe *inside* the `@vitejs/devtools` hub instead of standing alone. That gets Prose a dock entry alongside other DevTools panels (Vite Inspect, the plugin timeline, etc.) for free, at the cost of the hub's own dependency weight and its `clientAuth`-gated-by-default trust handshake (not a plain option on `DevTools()`, and not needed for this tool's audience-of-one target). Worth revisiting if Prose ever wants to sit in the same dock as other project tooling; until then, the standalone `@devframes/vite` bridge is the simpler fit.
