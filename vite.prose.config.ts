 ...
 * `vp fmt`/`vp lint`/`vp test` and has no dev server) and `vite.client.config.ts` (the client
 ...
 * `vite build --config vite.client.config.ts`) — `devframeVitePlugin` serves `client/dist`, not
